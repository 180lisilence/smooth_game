...
static char g_comm_dir[MAX_PATH] = {0};  /* 通信文件目录（DLL所在目录） */
...
static void check_command_file() {
    ...
    /* 使用DLL所在目录作为通信目录（不写C盘）
       DLL注入到游戏进程后，通过GetModuleFileNameA获取自身路径，
       通信文件放在DLL同目录下，Python端也写同一目录 */
    if (g_comm_dir[0] == 0) {
        /* 兜底：如果g_comm_dir未初始化，用LOCALAPPDATA */
        GetEnvironmentVariableA("LOCALAPPDATA", g_comm_dir, MAX_PATH);
        strcat(g_comm_dir, "\\woldvein_trainer");
    }
    CreateDirectoryA(g_comm_dir, NULL);
    snprintf(cmd_file, MAX_PATH, "%s\\lua_cmd.txt", g_comm_dir);
    snprintf(result_file, MAX_PATH, "%s\\lua_result.txt", g_comm_dir);
